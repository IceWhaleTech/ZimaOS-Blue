...
```
Temporizador (30 min) → Ler HEARTBEAT.md → Avaliação LLM → Remover token HEARTBEAT_OK
  → Deduplicação (hash FNV, TTL 24h) → Alerta de canal (Telegram/Slack/...)
  → Fluxo de eventos → Indicador UI
```
