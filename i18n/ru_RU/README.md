...
| Метрика | ZimaOS Blue (Go) | Reference Agent (Node + dist) |
|---------|-------------------|------------------------|
...
| память простоя `gateway run` после холодного старта | **~19 MB** | - |
| Зависимости среды выполнения | **Нет** | Node.js 18+ |

> Строки CLI выше — это исторический микробенчмарк `help` / `status` на том же хосте. Новая строка `gateway run` показывает реальную память простоя после холодного старта, измеренную на macOS arm64 через `vmmap Physical footprint` после стабилизации запуска. Фев-Апр 2026.
...
**Поток оценки Harness**
```
Quick Eval / Harness API → Контроллер оценки → Диспетчер групп запусков
  → Задача агента или eval driver → Инструменты + Workspace + Artifacts
  → Scorecards / Reports / Budget+Execution+Selector Gates
  → Cutover Readiness / Решение по кандидату
```
