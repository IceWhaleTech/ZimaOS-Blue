# v0.10.9 Ngrok Remote Access PRD (Updated with v0.10.10 Improvements)

## Overview

Version 0.10.9 introduces ngrok-based remote access functionality using the **ngrok-go SDK**, allowing users to access their ZimaOS Echo instance from anywhere via a simple QR code scan. The SDK is embedded directly into the application, eliminating the need for external binary downloads.
...
listener, err := ngrok.Listen(ctx,
    config.HTTPEndpoint(
        config.WithForwardsTo("localhost:8080"),
    ),
...
**Requirements**:
- Start tunnel with configurable port (default: 8080)
...
- **User Message**: "Port 8080 is already in use. Please stop other services or configure a different port."
