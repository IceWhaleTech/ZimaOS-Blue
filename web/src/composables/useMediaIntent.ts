...
const allLangKeywords: Record<string, LangKeywords> = {
  en: {
    actions: ['generate', 'create', 'draw', 'paint', 'make', 'produce', 'render', 'design', 'sketch'],
    imageNouns: ['image', 'picture', 'photo', 'photograph', 'illustration', 'artwork', 'poster', 'wallpaper', 'portrait', 'icon', 'logo', 'banner'],
    videoNouns: ['video', 'animation', 'clip', 'movie', 'film', 'motion'],
    editVerbs: ['edit', 'change', 'modify', 'transform', 'alter', 'adjust', 'fix', 'retouch', 'enhance', 'upscale', 'remove', 'replace', 'restyle', 'photoshop', 'touch up', 'touchup', 'crop', 'inpaint', 'outpaint', 'colorize', 'sharpen', 'denoise', 'swap face', 'restore', 'cleanup', 'clean up'],
  ...
  zh: {
    ...
    imageNouns: ['图', '图片', '照片', '图像', '插画', '海报', '壁纸', '头像', '图标', 'logo', '画'],
    videoNouns: ['视频', '动画', '影片', '短片', '动图', '视频片段'],
    editVerbs: ['编辑', '修改', '改', '变', '调整', '修复', '美化', '去除', '替换', '换', '修图', '改图', 'p图', '抠图', '磨皮', '滤镜', '换脸', '换背景', '去水印', '美颜', '瘦脸', '祛痘', '补光'],
  ...
  ja: {
    ...
    editVerbs: ['編集', '変更', '修正', '加工', '調整', '変えて', '直して', 'レタッチ', '修整', '補正', '切り抜き', '合成'],
  ...
  ko: {
    ...
    editVerbs: ['편집', '수정', '변경', '바꿔', '고쳐', '조정', '보정', '리터치', '합성', '배경제거'],
...
export function classifyMediaIntent(
  ...
  if (imageCount >= 2 && (hasVideoNoun || hasAnimVerb)) {
    ...
    category = 'i2i'; confidence = 0.9
  } else if (hasImages && hasAction && hasImageNoun) {
    category = 'i2i'; confidence = 0.7
  } else if (hasAction && hasVideoNoun) {
    category = 't2v'; confidence = 0.85
  } else if (hasVideoNoun && !hasAction) {
    category = 't2v'; confidence = 0.6
  } else if (hasAction && hasImageNoun) {
    category = 't2i'; confidence = 0.85
  } else if (hasImageNoun && !hasAction) {
    category = 't2i'; confidence = 0.5
  }
