...
describe('Deep research cards', () => {
  it('renders verification summary and research trace details', () => {
    const wrapper = mount(CardDeepResearch, {
      props: {
        card: {
          ...
          latest_gap: 'Resolve conflicting claims',
    ...
    expect(wrapper.text()).toContain('Research trace')
    expect(wrapper.text()).toContain('Iterations: 2')
    expect(wrapper.text()).toContain('Coverage target reached')
    ...
    expect(wrapper.text()).toContain('Conflicted')
  })

  it('renders verify-stage progress with iteration and latest gap', () => {
