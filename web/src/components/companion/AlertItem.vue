...
const severityConfig = computed(() => {
  const configs: Record<AlertSeverity, { icon: string; color: string; bg: string }> = {
    ...
    info: {
      ...
      color: 'text-gray-900 dark:text-white dark:text-white',
      bg: 'bg-gray-700 dark:bg-gray-500/30 border-gray-900 dark:border-white dark:border-gray-900 dark:border-white',
    },
    error: {
      icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
      color: 'text-red-600 dark:text-red-400',
...
<template>
  ...
  >
    <div class="flex items-start gap-3">
      <!-- Severity Icon -->
      <div
        :class="['flex-shrink-0 p-2 rounded-full bg-white dark:bg-gray-700', severityConfig.color]"
      >
        <svg
      ...
      <div class="flex-1 min-w-0">
        ...
        <h4
          :class="['font-medium text-gray-900 dark:text-white', compact ? 'text-sm' : 'text-base']"
        >
          {{ alert.title }}
        </h4>

        <p
          v-if="alert.description && !compact"
        ...
        <!-- Acked info -->
        <div
          v-if="alert.acknowledged && alert.acked_by"
