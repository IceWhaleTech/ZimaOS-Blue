...
<template>
  ...
  >
    <div class="flex">
      ...
      <!-- Loading placeholder -->
      <div v-else-if="isLoading" class="flex-shrink-0 w-32 h-24 bg-gray-100 dark:bg-gray-700 animate-pulse" />

      <!-- Content -->
      <div class="flex-1 p-4 min-w-0">
        <!-- Site info -->
        <div class="flex items-center gap-2 mb-2">
          <img v-if="displayFavicon" :src="displayFavicon" class="w-4 h-4" :alt="displaySiteName || ''" />
          <div v-else-if="isLoading" class="w-4 h-4 bg-gray-200 dark:bg-gray-600 rounded animate-pulse" />
        ...
        <!-- Description -->
        <p v-if="displayDescription && !isLoading" class="mt-1 text-sm text-gray-500 dark:text-gray-400 line-clamp-2">
          {{ displayDescription }}
        </p>
