...
<template>
  <div class="media-generate-card rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800">
    <!-- Generating: dreamy gradient placeholder -->
    <div v-if="isGenerating" class="relative">
      <div class="generating-placeholder w-full aspect-[4/3] max-h-80 flex items-center justify-center">
        <div class="text-center z-10">
          <div class="mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-white/80 mx-auto animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
          </div>
          <p class="text-white/90 text-sm font-medium">{{ card.media_type === 'video' ? t('chat.generatingVideo', 'Generating video...') : t('chat.generatingImage', 'Generating image...') }}</p>
          <p class="text-white/60 text-xs mt-1">{{ t('chat.pleaseWait', 'This may take 15-60 seconds') }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="isError" class="p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('chat.generationFailed', 'Generation failed') }}</p>
        <p v-if="displayCard.message" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ displayCard.message }}</p>
      </div>
    </div>
    ...
    <!-- Lightbox -->
    <Teleport to="body">
      <div v-if="selectedImage" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90" @click="closeLightbox">
        <button class="absolute top-4 right-4 p-2 text-white hover:bg-white/10 rounded-full transition-colors" @click.stop="closeLightbox">
        ...
        <div class="max-w-4xl max-h-[90vh] p-4" @click.stop>
          ...
          <p v-if="selectedImage.caption" class="mt-3 text-center text-white text-sm">{{ selectedImage.caption }}</p>
        </div>
      </div>
    </Teleport>
  </div>
