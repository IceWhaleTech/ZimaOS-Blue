...
const majorCount = computed(() => issues.value.filter((i: UIReviewIssue) => i.severity === 'major').length)
const minorCount = computed(() => issues.value.filter((i: UIReviewIssue) => i.severity === 'minor').length)
...
<template>
  <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden shadow-sm">
    ...
    <div v-if="isError" class="p-4">
      ...
      <p v-if="card.message" class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ card.message }}</p>
    ...
    <template v-else>
      ...
      >
        <!-- Eye icon -->
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-if="expanded" class="border-t border-gray-100 dark:border-gray-700">
          ...
          <div class="px-4 py-3 space-y-2 border-t border-gray-100 dark:border-gray-700">
            <div v-if="card.visual && card.visual.score > 0" class="flex items-center gap-3">
              <span class="text-xs text-gray-500 dark:text-gray-400 w-24">{{ t('uiReview.visual', 'Visual') }}</span>
              <div class="flex-1 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full rounded-full transition-all" :class="scoreBarColor(card.visual.score)" :style="{ width: scoreBarWidth(card.visual.score) }" />
              </div>
              <span class="text-xs font-medium tabular-nums text-gray-600 dark:text-gray-400 w-8 text-right">{{ card.visual.score.toFixed(0) }}</span>
            </div>
            <div v-if="card.functional" class="flex items-center gap-3">
              <span class="text-xs text-gray-500 dark:text-gray-400 w-24">{{ t('uiReview.functional', 'Functional') }}</span>
              <div class="flex-1 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full rounded-full transition-all" :class="scoreBarColor(card.functional.score)" :style="{ width: scoreBarWidth(card.functional.score) }" />
              </div>
              <span class="text-xs font-medium tabular-nums text-gray-600 dark:text-gray-400 w-8 text-right">{{ card.functional.score.toFixed(0) }}</span>
            </div>
            <div v-if="card.accessibility" class="flex items-center gap-3">
              <span class="text-xs text-gray-500 dark:text-gray-400 w-24">{{ t('uiReview.accessibility', 'Accessibility') }}</span>
              <div class="flex-1 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full rounded-full transition-all" :class="scoreBarColor(card.accessibility.score)" :style="{ width: scoreBarWidth(card.accessibility.score) }" />
              </div>
              <span class="text-xs font-medium tabular-nums text-gray-600 dark:text-gray-400 w-8 text-right">{{ card.accessibility.score.toFixed(0) }}</span>
            </div>
          </div>

          <!-- Issues -->
          <div v-if="issues.length > 0" class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <!-- Suggestions -->
          <div v-if="suggestions.length > 0" class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">
            <div class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
              {{ t('uiReview.suggestions', 'Suggestions') }}
            </div>
            <ul class="space-y-1">
              <li v-for="(s, i) in suggestions" :key="i" class="text-xs text-gray-500 dark:text-gray-400 flex items-start gap-1.5">
          ...
          <!-- Action buttons -->
          <div v-if="card.actions?.length" class="px-4 py-3 border-t border-gray-100 dark:border-gray-700 flex flex-wrap gap-2">
            <button
              v-for="action in card.actions"
              :key="action.id"
              class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
