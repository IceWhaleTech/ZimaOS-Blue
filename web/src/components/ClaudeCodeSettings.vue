...
import type {
  ClaudeCodeVersionResponse,
  CheckUpdateResponse,
  ClaudeCodeConfigResponse,
...
async function toggleSandbox() {
  ...
  try {
    ...
    emit(
      'status-change',
      newSandboxEnabled ? t('claudecode.sandboxEnabled') : t('claudecode.sandboxDisabled')
    )
  } catch (_e) {
    error.value = t('claudecode.toggleError')
    emit('status-change', t('claudecode.toggleError'))
...
async function toggleNetwork() {
  ...
  try {
    ...
    emit(
      'status-change',
      newNetworkEnabled ? t('claudecode.networkEnabled') : t('claudecode.networkDisabled')
    )
  } catch (_e) {
    error.value = t('claudecode.toggleError')
    emit('status-change', t('claudecode.toggleError'))
...
async function toggleWhitelist() {
  ...
  try {
    ...
    emit(
      'status-change',
      newWhitelistEnabled ? t('claudecode.whitelistEnabled') : t('claudecode.whitelistDisabled')
    )
  } catch (_e) {
    error.value = t('claudecode.toggleError')
    emit('status-change', t('claudecode.toggleError'))
...
async function checkForUpdates() {
  try {
    ...
    if (response.data.update_available) {
      emit(
        'status-change',
        t('claudecode.updateAvailable', { version: response.data.latest_version })
      )
    } else {
  ...
  <section class="mb-6 sm:mb-8" id="claude-code-settings">
    ...
    <div class="glass-card">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5 flex-shrink-0 text-gray-600 dark:text-gray-300"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            ...
            <div v-else-if="versionInfo">
              ...
              >
                <div class="flex items-start gap-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 text-blue-600 dark:text-blue-300 flex-shrink-0 mt-0.5"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
              ...
              <div v-if="!isInstalled" class="text-center py-6">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-12 w-12 mx-auto text-gray-400 dark:text-slate-500 mb-4"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                ...
                <p class="text-gray-500 dark:text-slate-400 mb-2">
                  {{ t('claudecode.notInstalledDescription') }}
                </p>
                <p class="text-sm text-gray-400 dark:text-slate-500 mb-4">
                  {{ t('claudecode.platform') }}: {{ versionInfo.platform }}
                </p>
                <button
                ...
                <div
                  v-if="error"
                  class="bg-red-50 dark:bg-red-900/30 border border-red-300 dark:border-red-600 rounded-lg p-3 mt-4 mx-auto max-w-md"
                >
              ...
              <template v-else>
                ...
                <div class="flex items-center justify-between mb-4">
                  ...
                  <span
                    v-if="sourceLabel"
                    class="text-sm px-2 py-1 rounded-full bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300"
                  >
                    {{ sourceLabel }}
                  </span>
                </div>
                ...
                <div
                  class="mb-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg divide-y divide-gray-200 dark:divide-slate-600"
                >
                  <!-- Sandbox Toggle -->
                  <div class="flex items-center justify-between p-3">
                    <div class="flex items-center gap-3">
                      <span class="text-gray-900 dark:text-white font-medium">{{
                        t('claudecode.sandboxMode')
                      }}</span>
                      <span class="text-xs text-gray-500 dark:text-slate-400">{{
                        t('claudecode.sandboxModeDesc')
                      }}</span>
                    </div>
                    <button
                  ...
                  <div class="flex items-center justify-between p-3">
                    <div class="flex items-center gap-3">
                      <span class="text-gray-900 dark:text-white font-medium">{{
                        t('claudecode.networkAccess')
                      }}</span>
                      <span class="text-xs text-gray-500 dark:text-slate-400">{{
                        t('claudecode.networkAccessDesc')
                      }}</span>
                    </div>
                    <button
                  ...
                  <div class="flex items-center justify-between p-3">
                    <div class="flex items-center gap-3">
                      <span class="text-gray-900 dark:text-white font-medium">{{
                        t('claudecode.directoryWhitelist')
                      }}</span>
                      <span class="text-xs text-gray-500 dark:text-slate-400">{{
                        t('claudecode.directoryWhitelistDesc')
                      }}</span>
                    </div>
                    <button
                ...
                <Transition name="collapse">
                  <div
                    ...
                    class="mb-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg p-3"
                  >
                        ...
                        <template v-if="editingIndex !== index">
                          <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                              ...
                              >
                              <span
                                class="text-sm text-gray-600 dark:text-slate-300 font-mono truncate"
                                :class="{ 'text-gray-400 dark:text-slate-500': entry.alias }"
                                >{{ entry.path }}</span
                              >
                            </div>
                          </div>
                    ...
                    <div
                      class="flex flex-col sm:flex-row gap-2 pt-3 border-t border-gray-200 dark:border-slate-600"
                    >
                      <div class="flex-1 flex gap-1">
                        <input
                          v-model="newDirPath"
                  ...
                  <div class="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      ...
                      <p class="text-gray-900 dark:text-white dark:text-white text-sm">
                        {{ t('claudecode.latestVersion') }}:
                        {{ updateInfo?.latest_version || t('common.unknown') }}
                      </p>
                    </div>
                    <button
                      :disabled="updating"
                ...
                <!-- Last Check -->
                <p
                  v-if="versionInfo.last_check"
                  class="text-xs text-gray-500 dark:text-slate-400 mb-4"
                >
                ...
                <div
                  v-if="error"
                  class="bg-red-50 dark:bg-red-900/30 border border-red-300 dark:border-red-600 rounded-lg p-3 mb-4"
                >
                  <p class="text-red-800 dark:text-red-200 text-sm">{{ error }}</p>
                </div>
    ...
    <Teleport to="body">
      <Transition name="fade">
        ...
        >
          <div
            class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg max-h-[70vh] flex flex-col border border-gray-200 dark:border-slate-600"
          >
            <!-- Header -->
            <div
              class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-slate-600"
            >
              <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
                {{ t('claudecode.browseDirectories') }}
              </h3>
              <button
            ...
            <!-- Current path -->
            <div
              class="px-4 py-2 bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-600"
            >
              <p
                class="text-xs text-gray-500 dark:text-slate-400 font-mono truncate"
                :title="browserCurrent"
              >
                {{ browserCurrent }}
              </p>
            </div>
            ...
            <div class="flex-1 overflow-y-auto min-h-0">
              ...
              <div v-else class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <!-- Empty state -->
                <div
                  v-if="!browserDirs.length && !browserParent"
                  class="text-center py-6 text-gray-400 dark:text-slate-500 text-sm"
                >
