...
const currentModel = computed(() => status.value?.asr?.model_type ?? '')
  ...
  try {
    ...
    const pollInterval = setInterval(async () => {
      ...
      if (res.data?.progress) {
        downloadProgress.value = res.data.progress.percentage
      }
...
async function deleteModel(modelType?: string) {
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
      ...
      <div v-else-if="status" class="space-y-4">
        ...
        <div v-if="currentModel" class="flex items-center justify-between">
          <span class="text-gray-600 dark:text-gray-400">{{ t('speech.currentModel') }}</span>
          <span class="font-medium text-gray-900 dark:text-white">{{ currentModel }}</span>
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <span
            :class="status.tts?.ready ? 'text-green-500' : 'text-yellow-500'"
            class="font-medium"
          >
            {{ status.tts?.ready ? t('speech.ready') : t('speech.notReady') }}
          </span>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
      ...
      <div v-if="downloading" class="mb-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-sm font-medium text-gray-900 dark:text-white">{{ downloadProgress }}%</span>
        </div>
