  ...
  <div class="space-y-6">
    ...
    <div class="glass-card p-6">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $t('settings.update.title') }}</h3>
    ...
    <!-- Settings Card -->
    <div class="glass-card p-6">
      <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-4">{{ $t('common.settings') }}</h4>
      <div class="space-y-4">
        <div class="flex items-center justify-between">
        ...
        <div class="flex items-center justify-between">
          <span class="text-sm text-gray-700 dark:text-gray-300">{{ $t('settings.update.autoDownload') }}</span>
          <button
            type="button"
            role="switch"
...
const checkUpdate = async () => {
  ...
  try {
    ...
    if (res.data.update_available) {
      ...
      if (autoDownload.value) downloadUpdate()
    } else {
...
const saveSettings = () => {
  ...
  localStorage.setItem('update_auto_download', String(autoDownload.value))
}

onMounted(async () => {
  autoCheck.value = localStorage.getItem('update_auto_check') !== 'false'
  autoDownload.value = localStorage.getItem('update_auto_download') === 'true'
  await fetchInfo()
