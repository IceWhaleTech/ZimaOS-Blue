...
// Validation
const exportPasswordsMatch = computed(() => exportPassword.value === exportConfirmPassword.value)

const canExport = computed(() => exportPassword.value.length >= 6 && exportPasswordsMatch.value)

const canImport = computed(() => importPassword.value.length >= 6 && importFile.value !== null)

// Cleanup computed
const hasCleanupTargets = computed(() => Object.values(cleanupTargets.value).some((v) => v))
...
const activeModalTitle = computed(() => {
  switch (activeTab.value) {
    case 'export':
      return t('userdata.export')
    case 'import':
      return t('userdata.import')
    case 'cleanup':
      return t('userdata.tabs.cleanup')
    default:
      return t('userdata.chatDataTitle', 'Chat Data')
  }
})
...
async function executeCleanup() {
  ...
  try {
    ...
    if (
      cleanupTargets.value.sessions ||
      cleanupTargets.value.events ||
      cleanupTargets.value.alerts
    ) {
...
<template>
  <div class="w-full">
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[86vh] overflow-y-auto"
        >
          <div
