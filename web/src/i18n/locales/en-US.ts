...
export default {
  ...
  "harness": {
    "builder": {
      ...
      "description": "Reach for the full dataset, version, spec, and run workflow when you need exact control over every eval object.",
      "collapsed": "Advanced mode is collapsed. Open it when you need to hand-author datasets, immutable versions, eval specs, or run metadata."
    },
    "datasets": {
      "eyebrow": "Reusable Sources",
      "title": "Datasets & versions",
      "description": "Each dataset can carry multiple frozen versions, letting specs rerun against the same manifest later.",
    ...
    "evalSpecs": {
      ...
      "title": "Eval specs",
      "description": "Specs capture one dataset snapshot plus the run kind, profile, and scoring setup needed to materialize repeatable runs.",
    ...
    "evalRun": {
      ...
      "reportDescription": "This preview joins the V3 eval run record with the underlying group report so you can inspect outcomes without leaving the console.",
    ...
    "evalRuns": {
      ...
      "title": "Eval runs",
      "description": "Run rows keep the V3 entrypoint visible while still linking back to the underlying group report and raw runtime trail.",
      "total": "Eval runs"
    },
    "baseline": {
      ...
      "description": "Pin the selected eval run as a reusable baseline, then compare future candidates against it from the same console.",
    ...
    "compare": {
      ...
      "description": "Generate a persisted comparison report against a named baseline or another eval run from the same spec.",
    ...
    "group": {
      ...
      "scorecardInsightsHint": "Harness surfaces scoring diagnostics here; proposal review still happens in Memory / Self-evolution.",
      "reviewProposals": "Review in Memory",
      "noScorecardInsights": "No calibration or proposal diagnostics were attached to the current scorecards.",
      "remediationMissingArtifact": "Attach the expected artifact path or label to the run, or align expected_artifacts with the actual output contract.",
      "remediationRequiredCheckMissing": "Emit a stable completion marker in events or results, or align required_checks with the observable success signal.",
      "remediationToolSelectionError": "Make the required tool available in this profile and bias planning so the run selects it explicitly.",
      ...
      "remediationRunMissing": "Check scheduler persistence and run linking so the produced run is stored before scoring begins.",
      "remediationRunFailed": "Inspect the linked run error and logs, fix the runtime failure, then retry the case.",
      "remediationRunCancelled": "Check what cancelled the run and keep approvals or user-input waits from terminating the attempt early.",
      "remediationRunAborted": "Inspect abort reasons from the driver or runtime and restore the execution path before retrying.",
      "remediationRunNotCompleted": "Reduce task scope or raise runtime budgets so the run can reach completed status and publish its evidence.",
      "remediationVerificationFailed": "Add stronger observable evidence or artifacts for success, or relax the contract if it is stricter than the intended outcome.",
      "remediationTimeout": "Shorten the task or increase timeout-related budgets so required steps can finish before scoring.",
      "remediationGeneric": "Inspect linked runs, checks, and artifacts to align the runtime output with the declared contract.",
    ...
    "quickEval": {
      ...
      "conversationHint": "Generate draft cases by pairing each user message with the next assistant reply.",
      ...
      "description": "Keep input minimal: choose cases, pick a preset, and Harness auto-creates the dataset version, eval spec, and run for you.",
      "draftCases": "Draft cases",
      "editManifest": "Edit Cases JSON",
      "eyebrow": "Default Path",
      "launch": "Launch quick eval",
      "launchHint": "Start from pasted cases, a past conversation, or an existing dataset. Harness fills in the object model behind the scenes.",
      "minimalDescription": "Most runs only need the case source and a preset. Everything else becomes an implementation detail instead of a required form.",
      ...
      "systemWillDoHint": "Create or reuse the dataset snapshot, choose a matching spec, attach the default baseline, and launch the run.",
      "title": "Quick Eval",
  ...
  "chat": {
    ...
    "processTrace": {
      ...
      "details": {
        ...
        "networkInterruptWaiting": "The stream was interrupted after content started. Waiting for the backend to recover.",
  ...
  "plugins": {
    ...
    "skillUrlHint": "Supports GitHub raw/blob SKILL.md links, tree directory links, and direct package URLs. Tree links install the full skill directory.",
  ...
  "security": {
    ...
    "directoryWhitelistHint": "Use aliases like {'@'}docs/... in tool calls. Paths must be absolute; aliases may contain letters, numbers, \"-\" and \"_\".",
  ...
  "skillStore": {
    ...
    "marketplace": {
      ...
      "results": {
        ...
        "syncHint": "The catalog refreshes automatically once per day. Use Refresh sources when you need immediate updates.",
        "catalog": "Catalog",
  ...
  "userdata": {
    ...
    "retention": {
      ...
      "description": "Set how long security monitoring data (sessions, events, alerts) is kept before automatic cleanup.",
  ...
  "speech": {
    ...
    "voiceWake": {
      ...
      "targetDescription": "After Voice Wake is triggered, every command is always sent to this fixed conversation instead of following the currently open chat.",
