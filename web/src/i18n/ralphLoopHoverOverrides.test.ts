...
import ralphLoopHoverOverrides from './ralph-loop-hover-overrides'
...
const requiredKeys = [
  ...
  'chat.ralphLoopHoverPlan',
  'chat.ralphLoopHoverAct',
  'chat.ralphLoopHoverCheck',
] as const

describe('ralph loop hover localization coverage', () => {
  it('defines ralph loop hover copy for all 27 locales', () => {
    const overrides = ralphLoopHoverOverrides as Record<string, LocaleMessages>
    ...
    for (const locale of overrideLocales) {
      ...
      expect(localeMessages, `${locale} should define ralph loop hover overrides`).toBeTruthy()
