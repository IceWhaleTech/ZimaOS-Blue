...
const selectedPanel = ref<'schema' | 'log'>('schema')
...
function scrollToPanel(panel: 'schema' | 'log') {
...
const recentIngestCount = computed(
  () => recentActivity.value.filter((entry) => entry.operation === 'ingest').length
)
...
const summaryCards = computed(() => [
  {
    key: 'pages',
    label: tr('knowledge.pageList', 'Pages'),
    value: String(pages.value.length),
    details: trp('knowledge.pageCount', '{count} pages', { count: pages.value.length }),
  },
  {
    key: 'ingests',
    label: tr('knowledge.recentIngests', 'Recent ingests'),
    value: String(recentIngestCount.value),
    details:
      recentActivity.value.find((entry) => entry.operation === 'ingest')?.title ||
      tr('knowledge.noRecentIngests', 'No recent ingests yet.'),
  },
  {
    key: 'conflicts',
    label: tr('knowledge.unresolvedConflicts', 'Unresolved conflicts'),
    value: String(conflictIssueCount.value),
    details: trp('knowledge.conflictCount', '{count} unresolved conflicts', {
      count: conflictIssueCount.value,
    }),
  },
  {
    key: 'gaps',
    label: tr('knowledge.openGaps', 'Open gaps'),
    value: String(gapIssueCount.value),
    details: trp('knowledge.gapCount', '{count} open gaps', { count: gapIssueCount.value }),
  },
  {
    key: 'lint',
    label: tr('knowledge.latestLint', 'Last lint'),
    value: latestLint.value ? formatDate(latestLint.value.generated_at) : tr('knowledge.never', 'Never'),
    details:
      latestLint.value?.issues?.[0]?.message ||
      tr('knowledge.lintUnavailable', 'No lint report yet.'),
  },
])
...
<template>
  <div class="knowledge-page dashboard-page-frame">
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-4">
      <AutomationTabs class="automation-tab-strip" />

      <section
        class="overflow-hidden rounded-3xl border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(245,158,11,0.18),_transparent_32%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.14),_transparent_38%),linear-gradient(135deg,_rgba(255,255,255,0.98),_rgba(248,250,252,0.96))] shadow-sm"
      >
        <div class="border-b border-slate-200/80 px-4 py-5 sm:px-5">
          <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span
                  data-testid="knowledge-beta-badge"
              ...
              <h1 class="text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">
                {{
                  tr(
                    'knowledge.title',
                    'Incremental ingest, durable wiki pages, and query-to-knowledge promotion'
                  )
                }}
              </h1>
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            >
              ...
              <div class="mt-2 text-xl font-semibold text-slate-950 sm:text-2xl">
                {{ card.value }}
              </div>
      ...
      <div class="grid gap-4 xl:grid-cols-[18rem_minmax(0,1fr)]">
        <aside class="space-y-4">
          <section class="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
            ...
            <div
              v-if="latestIngest"
              class="mt-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-3 text-xs text-slate-600"
            >
          ...
          >
            <div class="flex items-center justify-between gap-3">
              ...
              <span
                ...
                :class="selectedPanel === 'schema' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'"
              >
        ...
        <div class="space-y-4">
          <section class="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
            ...
            <div class="mt-4 grid gap-4 xl:grid-cols-2">
              ...
              >
	                <div class="flex items-center justify-between gap-3">
	                  <h3 class="text-sm font-semibold uppercase tracking-[0.14em] text-slate-600">
          ...
          <section class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <span
                  class="rounded-full px-2.5 py-1 text-[11px] font-medium"
                  :class="selectedPanel === 'log' ? 'bg-sky-100 text-sky-700' : 'bg-slate-100 text-slate-600'"
                >
          ...
          <section class="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
            ...
            <div v-else-if="selectedPage" class="space-y-4">
              <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div class="flex flex-wrap items-center gap-2">
                    <h2 class="text-xl font-semibold text-slate-950 sm:text-2xl">
                      {{ selectedPage.title }}
                    </h2>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-3">
                  ...
                  <div class="mt-3 space-y-2">
                    <div
                      ...
                      :key="answer.path"
                      class="rounded-2xl bg-white px-3 py-2 text-sm text-slate-700 shadow-sm"
                    >
              ...
              <div class="rounded-2xl border border-slate-200 bg-slate-950 p-4 shadow-inner">
                ...
                <pre class="mt-3 overflow-x-auto whitespace-pre-wrap text-sm leading-5 text-slate-100">{{
