...
LOG = logging.getLogger("execution-gate-artifact-fetcher")
...
class GitHubActionsClient:
    ...
    def _raw_request(self, method: str, path: str) -> bytes:
        headers = {
            ...
            "User-Agent": "execution-gate-artifact-fetcher",
        }
        if self.token:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download execution gate report artifacts from prior GitHub Actions workflow runs.")
    parser.add_argument(
        "--repo",
    ...
    parser.add_argument(
        ...
        help="Optional GitHub token used for Actions API access.",
    )
    parser.add_argument(
        "--api-base-url",
    ...
    parser.add_argument(
        "--workflow",
        default="execution-gate.yml",
        help="Workflow file name or workflow id used to list historical runs.",
    )
    parser.add_argument(
        "--branch",
    ...
    parser.add_argument(
        "--artifact-name",
        default="execution-gate-report",
        help="Artifact name to download from matching workflow runs.",
    )
    parser.add_argument(
        "--exclude-run-id",
        default=os.environ.get("GITHUB_RUN_ID", ""),
        help="Optional workflow run id to exclude from history fetches.",
    )
    parser.add_argument(
        "--limit-runs",
        type=int,
        default=10,
        help="Maximum number of completed workflow runs to inspect after exclusions.",
    )
    parser.add_argument(
        "--output-dir",
        default="docs/reports/execution_gate_history_artifacts",
        help="Directory where fetched artifacts will be extracted.",
    )
    parser.add_argument(
        "--output-json",
        default="docs/reports/execution_gate_history_artifacts/manifest.json",
        help="Path for the structured fetch manifest.",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
def stage_execution_report_files(output_dir: Path, destination: Path, extracted_files: List[str], run_id: str, artifact_id: str) -> List[str]:
    staged: List[str] = []
    report_dir = output_dir / "execution_reports"
...
def fetch_execution_gate_history(client: GitHubActionsClient, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for workflow_run in workflow_runs:
        ...
        for artifact in artifacts:
            ...
            execution_report_files = stage_execution_report_files(output_dir, destination, extracted_files, run_id, artifact_id)
            downloaded.append(
                {
                    "run_id": run_id,
                    ...
                    "execution_report_files": execution_report_files,
                }
            )
    ...
    return {
        ...
        "downloaded_report_count": sum(len(entry["execution_report_files"]) for entry in downloaded),
...
def main() -> int:
    ...
    try:
        ...
        manifest = fetch_execution_gate_history(client, args)
    except Exception as exc:
        LOG.error("execution gate artifact fetch failed: %s", exc)
        return 1

    write_manifest(manifest, Path(args.output_json))
    LOG.info(
        "downloaded %d execution gate artifacts from %d workflow runs",
