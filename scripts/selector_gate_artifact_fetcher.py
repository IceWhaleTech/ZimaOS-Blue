...
LOG = logging.getLogger("selector-gate-artifact-fetcher")
...
class GitHubActionsClient:
    ...
    def _raw_request(self, method: str, path: str) -> bytes:
        headers = {
            ...
            "User-Agent": "selector-gate-artifact-fetcher",
        }
        if self.token:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download selector gate report artifacts from prior GitHub Actions workflow runs.")
    parser.add_argument(
        "--repo",
    ...
    parser.add_argument(
        "--output-dir",
        default="docs/reports/selector_gate_history_artifacts",
        help="Directory where fetched artifacts will be extracted.",
    )
    parser.add_argument(
        "--output-json",
        default="docs/reports/selector_gate_history_artifacts/manifest.json",
...
def stage_selector_report_files(output_dir: Path, destination: Path, extracted_files: List[str], run_id: str, artifact_id: str) -> List[str]:
    staged: List[str] = []
    report_dir = output_dir / "selector_reports"
...
def fetch_selector_gate_history(client: GitHubActionsClient, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for workflow_run in workflow_runs:
        ...
        for artifact in artifacts:
            ...
            selector_report_files = stage_selector_report_files(output_dir, destination, extracted_files, run_id, artifact_id)
            downloaded.append(
                {
                    "run_id": run_id,
                    ...
                    "selector_report_files": selector_report_files,
                }
            )
    ...
    manifest = {
        ...
        "downloaded_report_count": sum(len(entry["selector_report_files"]) for entry in downloaded),
...
def main() -> int:
    ...
    try:
        ...
        manifest = fetch_selector_gate_history(client, args)
    except Exception as exc:
        LOG.error("selector gate artifact fetch failed: %s", exc)
        return 1

    write_manifest(manifest, Path(args.output_json))
    LOG.info(
        "downloaded %d selector gate artifacts from %d workflow runs",
