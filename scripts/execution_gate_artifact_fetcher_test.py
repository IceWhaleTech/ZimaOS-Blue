...
import execution_gate_artifact_fetcher as fetcher
...
class ExecutionGateArtifactFetcherTest(unittest.TestCase):
    ...
    def test_fetch_execution_gate_history_downloads_matching_artifacts_and_writes_manifest(self):
        matching_report = json.dumps(
            {
                ...
                "status": "passed",
                "eval_run": {"id": "run-211"},
                "execution_gate": {"passed": True, "metrics": {"pass_rate": 1.0}},
            }
        )
        stale_report = json.dumps(
            {
                ...
                "status": "failed",
                "eval_run": {"id": "run-210"},
                "execution_gate": {"passed": False, "metrics": {"pass_rate": 0.7}},
            }
        )
        ...
        client = StubGitHubActionsClient(
            workflow_pages={
                1: [
                    {"id": 211, "run_number": 9, "run_attempt": 1, "head_branch": "main", "display_title": "Execution Gate", "created_at": "2026-03-28T00:00:00Z", "updated_at": "2026-03-28T00:05:00Z"},
                    {"id": 210, "run_number": 8, "run_attempt": 1, "head_branch": "main", "display_title": "Execution Gate", "created_at": "2026-03-27T00:00:00Z", "updated_at": "2026-03-27T00:05:00Z"},
                ]
            },
            run_artifacts={
                "211": [
                    {"id": 501, "name": "execution-gate-report", "expired": False, "size_in_bytes": 1200},
                    {"id": 502, "name": "other-report", "expired": False, "size_in_bytes": 400},
                ],
                "210": [
                    {"id": 503, "name": "execution-gate-report", "expired": True, "size_in_bytes": 1000},
                    {"id": 504, "name": "execution-gate-report", "expired": False, "size_in_bytes": 1100},
                ],
            },
            artifact_payloads={
                "501": make_zip(
                    {
                        "docs/reports/execution_gate_report.json": matching_report,
                        "docs/reports/execution_gate_history_report.json": history_summary,
                        "docs/reports/execution_gate_report.md": "# current\n",
                    }
                ),
                "504": make_zip(
                    {
                        "docs/reports/execution_gate_report.json": stale_report,
                    }
                ),
            },
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            args = SimpleNamespace(
                ...
                exclude_run_id="",
                limit_runs=5,
                artifact_name="execution-gate-report",
                output_dir=str(root / "history"),
                output_json=str(root / "history" / "manifest.json"),
            )

            manifest = fetcher.fetch_execution_gate_history(client, args)
            ...
            current_report = root / "history" / "run-211-artifact-501" / "docs" / "reports" / "execution_gate_report.json"
            previous_report = root / "history" / "run-210-artifact-504" / "docs" / "reports" / "execution_gate_report.json"
            staged_current = root / "history" / "execution_reports" / "run-211-artifact-501-execution_gate_report.json"
            staged_previous = root / "history" / "execution_reports" / "run-210-artifact-504-execution_gate_report.json"
            skipped_history_summary = root / "history" / "execution_reports" / "run-211-artifact-501-execution_gate_history_report.json"
    ...
    def test_extract_artifact_zip_skips_unsafe_members(self):
        raw_zip = make_zip(
            {
                "../../escape.json": "{}",
                "/absolute/path.json": "{}",
                "docs/reports/execution_gate_report.json": "{}",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(extracted, ["docs/reports/execution_gate_report.json"])
            self.assertTrue((destination / "docs" / "reports" / "execution_gate_report.json").is_file())
