...
<#
.SYNOPSIS
    ZimaOS-Blue One-Click Installation Script for Windows
.DESCRIPTION
    Downloads and installs ZimaOS-Blue on Windows systems with full Windows Service support,
    including automatic recovery, Event Log integration, and proper service management.
.PARAMETER Version
    Version to install (default: latest)
.PARAMETER InstallDir
    Installation directory (default: C:\Program Files\ZimaOS-Blue)
.PARAMETER NoService
    Skip Windows service installation (run manually)
.PARAMETER Uninstall
    Uninstall ZimaOS-Blue
.EXAMPLE
    irm https://blue.zimaos.com/install.ps1 | iex
.EXAMPLE
...
param(
    ...
    [string]$InstallDir = "$env:ProgramFiles\ZimaOS-Blue",
    [switch]$NoService,
    [switch]$Uninstall
)
...
function Install-Binary {
    ...
    $binaryName = "blue-windows-$arch.zip"
    ...
    $tempFile = Join-Path $env:TEMP "zimaos-blue.zip"
    try {
...
function Install-WindowsService {
    ...
    $binaryPath = Join-Path $InstallDir "bin\blue.exe"
    $configPath = Join-Path $InstallDir "config\config.yaml"
    ...
    if ($service.Status -eq "Running") {
        ...
        # Log to Event Log
        try {
            Write-EventLog -LogName Application -Source $EventLogSource -EventId 1000 -EntryType Information -Message "ZimaOS Blue service installed and started successfully. Version: $Version"
        } catch {
...
function Add-FirewallRule {
    ...
    $ruleName = "ZimaOS-Blue"
    $existingRule = Get-NetFirewallRule -DisplayName $ruleName -ErrorAction SilentlyContinue

    if (-not $existingRule) {
        New-NetFirewallRule -DisplayName $ruleName `
            -Description "Allow ZimaOS-Blue web interface" `
            -Direction Inbound `
            -Protocol TCP `
            -LocalPort 23456 `
            -Action Allow `
...
function Write-Success {
    ...
    Write-Host "    Get-Service ZimaOS-Blue              - Check status"
    Write-Host "    Restart-Service ZimaOS-Blue          - Restart service"
    Write-Host "    Stop-Service ZimaOS-Blue             - Stop service"
    Write-Host "    Start-Service ZimaOS-Blue            - Start service"
    Write-Host ""
    Write-Host "  CLI Commands:" -ForegroundColor Yellow
    Write-Host "    blue.exe status                      - Check service status"
    Write-Host "    blue.exe restart                     - Restart service"
    Write-Host "    blue.exe --help                      - Show all commands"
    Write-Host ""
    Write-Host "  View Logs:" -ForegroundColor Yellow
    Write-Host "    Get-EventLog -LogName Application -Source ZimaOS-Blue -Newest 20"
    Write-Host "    Get-WinEvent -FilterHashtable @{LogName='Application';ProviderName='ZimaOS-Blue'} -MaxEvents 20"
    Write-Host ""
...
function Write-UninstallSuccess {
    ...
    Write-Host "╚═══════════════════════════════════════════╝" -ForegroundColor Green
    Write-Host ""
    Write-Host "  ZimaOS-Blue has been uninstalled."
    Write-Host ""
...
function Invoke-Install {
    ...
    if (-not $NoService) {
        ...
        Write-Info "To run manually: blue.exe --config `"$InstallDir\config\config.yaml`""
    }
...
function Invoke-Uninstall {
    ...
    Write-Info "Uninstalling ZimaOS-Blue..."
