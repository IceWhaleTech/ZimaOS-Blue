...
import selector_gate_artifact_fetcher as fetcher
...
class SelectorGateArtifactFetcherTest(unittest.TestCase):
    ...
    def test_fetch_selector_gate_history_downloads_matching_artifacts_and_writes_manifest(self):
        matching_report = json.dumps({"candidate_label": "batch-1", "passed": True, "status": "passed", "eval_run": {"id": "run-211"}})
        stale_report = json.dumps({"candidate_label": "batch-0", "passed": False, "status": "failed", "eval_run": {"id": "run-210"}})
        client = StubGitHubActionsClient(
            workflow_pages={
                1: [
                    {"id": 211, "run_number": 9, "run_attempt": 1, "head_branch": "main", "display_title": "Selector Gate", "created_at": "2026-03-28T00:00:00Z", "updated_at": "2026-03-28T00:05:00Z"},
                    {"id": 210, "run_number": 8, "run_attempt": 1, "head_branch": "main", "display_title": "Selector Gate", "created_at": "2026-03-27T00:00:00Z", "updated_at": "2026-03-27T00:05:00Z"},
                ]
            },
            run_artifacts={
                "211": [
                    {"id": 501, "name": "selector-gate-report", "expired": False, "size_in_bytes": 1200},
                    {"id": 502, "name": "other-report", "expired": False, "size_in_bytes": 400},
                ],
                "210": [
                    {"id": 503, "name": "selector-gate-report", "expired": True, "size_in_bytes": 1000},
                    {"id": 504, "name": "selector-gate-report", "expired": False, "size_in_bytes": 1100},
                ],
            },
            artifact_payloads={
                "501": make_zip(
                    {
                        "docs/reports/selector_gate_report.json": matching_report,
                        "docs/reports/selector_gate_report.md": "# current\n",
                    }
                ),
                "504": make_zip(
                    {
                        "docs/reports/selector_gate_report.json": stale_report,
                    }
                ),
            },
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            args = SimpleNamespace(
                ...
                exclude_run_id="",
                limit_runs=5,
                artifact_name="selector-gate-report",
                output_dir=str(root / "history"),
                output_json=str(root / "history" / "manifest.json"),
            )

            manifest = fetcher.fetch_selector_gate_history(client, args)
            ...
            current_report = root / "history" / "run-211-artifact-501" / "docs" / "reports" / "selector_gate_report.json"
            previous_report = root / "history" / "run-210-artifact-504" / "docs" / "reports" / "selector_gate_report.json"
            staged_current = root / "history" / "selector_reports" / "run-211-artifact-501-selector_gate_report.json"
            staged_previous = root / "history" / "selector_reports" / "run-210-artifact-504-selector_gate_report.json"
    ...
    def test_extract_artifact_zip_skips_unsafe_members(self):
        raw_zip = make_zip(
            {
                "../../escape.json": "{}",
                "/absolute/path.json": "{}",
                "docs/reports/selector_gate_report.json": "{}",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(extracted, ["docs/reports/selector_gate_report.json"])
            self.assertTrue((destination / "docs" / "reports" / "selector_gate_report.json").is_file())
