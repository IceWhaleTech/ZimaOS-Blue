...
12. **Frontend Change Evaluation via Chrome DevTools MCP** - When any frontend code (Vue components, styles, router, stores, or `web/` build output) is modified, spawn a subagent to evaluate and test the changes using Chrome DevTools MCP tools (e.g. `mcp__chrome-devtools__take_screenshot`, `mcp__chrome-devtools__navigate_page`, `mcp__chrome-devtools__take_snapshot`). This is only applicable when Chrome DevTools MCP is available in the environment (typically configured as `"chrome-devtools"` MCP server). The subagent should: (a) open the affected page in a browser tab, (b) take a screenshot and/or snapshot to visually verify the change, (c) check for JS console errors, and (d) report the result back before marking the task complete.
