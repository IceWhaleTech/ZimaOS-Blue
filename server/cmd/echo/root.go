...
var rootCmd = &cobra.Command{
	Use:   "echo",
	Short: "ZimaOS-Echo - NAS-Native Agent Runtime",
	Long: `ZimaOS-Echo is a NAS-native agent runtime that provides
...
func init() {
	...
	// Global flags
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default: ./config.yaml, ./config/, /etc/zimaos-echo/, ~/.zimaos-echo/)")
	rootCmd.PersistentFlags().BoolVar(&devMode, "dev", false, "isolate state under ~/.zimaos-echo-dev with shifted default ports")
	rootCmd.PersistentFlags().StringVar(&profile, "profile", "", "isolate state under ~/.zimaos-echo-<name>")
...
func initConfig() {
	...
	// 3. /etc/zimaos-echo/config.yaml
	// 4. $HOME/.zimaos-echo/config.yaml (fallback)
...
func getConfigDir() string {
	...
	if devMode {
		return filepath.Join(home, ".zimaos-echo-dev")
	}

	if profile != "" {
		return filepath.Join(home, ".zimaos-echo-"+profile)
	}

	return filepath.Join(home, ".zimaos-echo")
}
