...
func runServer() error {
	for {
		...
		if err := builtin.RegisterAll(skillRegistry); err != nil {
			return fmt.Errorf("failed to register built-in skills: %w", err)
		}
		...
		if err != nil {
			return fmt.Errorf("failed to initialize external auth service: %w", err)
		}
		...
		if !restartController.Requested() {
			return nil
		}
