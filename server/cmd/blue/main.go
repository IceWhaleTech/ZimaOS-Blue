...
func initRuntimeLogger(cfg *config.Config) error {
	logPath := filepath.Join(getLogsDir(), "blue.log")

	var initOpts []logger.InitOption
	if !sameLogOutputPath(cfg.Log.Output, logPath) {
		if err := os.MkdirAll(filepath.Dir(logPath), 0o750); err != nil {
			fmt.Fprintf(os.Stderr, "[WARN] Failed to create logs directory %s: %v\n", filepath.Dir(logPath), err)
		} else {
			logFile, err := os.OpenFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
			if err != nil {
				fmt.Fprintf(os.Stderr, "[WARN] Failed to open runtime log file %s: %v\n", logPath, err)
			} else {
				initOpts = append(initOpts, logger.WithAdditionalWriter(logFile))
			}
		}
	}

	if err := logger.Init(&cfg.Log, initOpts...); err != nil {
		return err
	}

	if len(initOpts) > 0 {
		logger.Info().Str("log_file", logPath).Msg("Runtime file logging enabled")
	}
	return nil
}

func sameLogOutputPath(configuredOutput, runtimeLogPath string) bool {
	output := strings.TrimSpace(configuredOutput)
	if output == "" || strings.EqualFold(output, "stdout") || strings.EqualFold(output, "stderr") {
		return false
	}

	configuredAbs, err := filepath.Abs(output)
	if err != nil {
		configuredAbs = filepath.Clean(output)
	}
	runtimeAbs, err := filepath.Abs(runtimeLogPath)
	if err != nil {
		runtimeAbs = filepath.Clean(runtimeLogPath)
	}
	return strings.EqualFold(configuredAbs, runtimeAbs)
}

func main() {
	pauseOnExit := newWindowsExplorerExitPauser(os.Args)
	exitCode := runMainWithPanicCapture()
	pauseOnExit(exitCode)
	os.Exit(exitCode)
}

func runMainWithPanicCapture() (exitCode int) {
	defer func() {
...
var runServerIteration = runServerDefaultIteration
...
func runServerDefaultIteration() serverRunOutcome {
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to load config: %w", err)}
	}
	var hotReloader *config.HotReloader

	// Initialize logger
	if err := initRuntimeLogger(cfg); err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize logger: %w", err)}
	}
	...
	if err := os.MkdirAll(dataDir, 0750); err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to create data directory: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize config kvstore: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize user repository: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize permission repository: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize memory store: %w", err)}
	}
	...
	if err := builtin.RegisterAll(skillRegistry); err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to register built-in skills: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize external auth service: %w", err)}
	}
	...
	if err != nil {
		return serverRunOutcome{Err: fmt.Errorf("failed to initialize API key service: %w", err)}
	}
...
func registerAPIRoutes(srv *server.Server, pool *worker.Pool, userHandler *user.Handler, extauthHandler *extauth.Handler, userService *user.Service, chatHandler *server.ChatHandler, autoreplyService *autoreply.Service, autoreplyHandler *autoreply.Handler, metricsCollector *metrics.Collector, metricsWriter *metrics.MetricsWriter, authMiddleware *auth.AuthMiddleware, apiKeyHandler *auth.APIKeyHandler, apiKeyService *auth.APIKeyService, skillRegistry *skill.Registry, backupHandler *backup.Handler, toolRegistry *tools.Registry, securityHandler *security.Handler, sandboxHandler *sandbox.Handler, sandboxManager *sandbox.Manager, cronHandler *cron.Handler, browserHandler *browser.Handler, workflowHandler *workflow.Handler, mfaHandler *mfa.Handler, voiceHandler *voice.Handler, voiceWSHandler *voice.WSHandler, formfillerHandler *formfiller.Handler, companionHandler *companion.Handler, companionWSHandler *companion.WebSocketHandler, ngrokTunnelMgr *ngrok.SDKTunnelManager, ngrokConfigStore *ngrok.ConfigStore, zapLogger *zap.Logger, version, buildTime, gitCommit, dataDir string, cfg *config.Config, llmRegistry *llm.ProviderRegistry, dbConn, runtimeConn *dbutil.SQLiteConn, db, dbReader *sql.DB, memoryStore *memory.Store, jwtService *auth.JWTService, permissionHandler *permission.Handler, sttService stt.Service, ttsService tts.Service, a2uiManager *a2ui.Manager, ocrService *ocrruntime.TesseractService, pdfService *pdfextract.Service, lm *lifecycle.Manager, hotReloader *config.HotReloader, sseBroker *ssePkg.Broker, pushIPC sockipc.PushBackend, pushSvc *push.Service, cronIPC sockipc.CronBackend, browserBackend tools.BrowserBackend, lazyBrowserSvc func() *browser.RodService, acquireBrowserSvc func() (*browser.RodService, func(), error), acquireFallbackBrowserSvc func() (*browser.RodService, func(), error), lightpandaShimSvc *browser.LightpandaService, configKV kvstore.Store, configStore *config.ConfigStore) {
	...
	speechService := speech.NewService(&speech.Config{
		TTS: speech.TTSConfig{Provider: string(defaultTTSProvider), Model: ""},
