...
var doctorCmd = &cobra.Command{
	...
	Long: `Run diagnostic checks on ZimaOS-Blue installation and configuration.
...
func checkPort() CheckResult {
	...
	if err != nil {
		...
		if err == nil {
			resp.Body.Close()
			result.Status = "pass"
			result.Message = fmt.Sprintf("Port %d in use by ZimaOS-Blue", port)
		} else {
			result.Status = "warn"
...
func printDoctorHuman(report DoctorReport) {
	fmt.Println("ZimaOS-Blue Doctor")
	fmt.Println("==================")
	fmt.Println()
