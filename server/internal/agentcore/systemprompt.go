...
func (b *SystemPromptBuilder) writeToolsInfoTo(sb *strings.Builder, hasSandbox bool) bool {
	...
	if len(nativeDocumentTools) > 0 {
		...
		sb.WriteString(" over convert, raw file_write, or one-off Python generators so structure, layout, and formatting are preserved. Do not use Python to create ")
		sb.WriteString(strings.Join(nativeDocumentTools, "/"))
	...
	if b.toolRegistry.Get("computer_use") != nil {
		sb.WriteString("<host_ui_guide>For application and window operations, prefer computer_use over exec or document tools so the runtime can inspect and act on native UI safely. For desktop chat or messaging tasks, prefer high-level computer_use scenario actions like `message`, `select`, and `type` before taking screenshots or issuing generic low-level actions. Do not invent unsupported computer_use action names like `activate`, `mouse_click`, `move_to`, `accessibility_tree`, or `ocr`. Also do not use `open`, `open_location`, `list_apps`, browser `read`, or top-level `press`; if you need a shortcut or submit key, use `key` with `keys`/`submit_keys`. For `message`, always include `value`. Use `screenshot` only for evidence capture or last-resort debugging, not as the default control path.</host_ui_guide>")
	}
