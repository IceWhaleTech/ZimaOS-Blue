...
// Directory structure: {skillsDir}/{skill-name}/{SKILL.md|CLAUDE.md|AGENT.md}
func ScanSkillsDir(skillsDir string) []SkillEntry {
	...
	for _, entry := range entries {
		...
		se, _, err := readSkillEntryFromDir(filepath.Join(skillsDir, entry.Name()))
		if err != nil {
			continue
		}
...
// Lower = appears first. Core public skills (web_query, browser, research)
...
// 1) workspace/.agents/skills (project-level, Agent Skills compatible)
// 2) workspace/.claude/skills (project-level, Claude-compatible)
