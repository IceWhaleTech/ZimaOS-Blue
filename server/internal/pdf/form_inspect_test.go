...
func TestServiceInspectFormReturnsTextFieldMetadata(t *testing.T) {
	...
	if result.FieldCount != 1 {
		t.Fatalf("field_count = %d, want 1", result.FieldCount)
	}
	if len(result.Fields) != 1 {
		t.Fatalf("len(fields) = %d, want 1", len(result.Fields))
	}
	if result.Document.Engine != engineName {
		t.Fatalf("document.engine = %q, want %q", result.Document.Engine, engineName)
	}

	field := result.Fields[0]
	if field.PageNumber != 1 {
		t.Fatalf("page_number = %d, want 1", field.PageNumber)
	}
	...
	if field.Rect == nil {
		t.Fatal("expected field rect metadata")
	}
	if field.Rect.Right <= field.Rect.Left || field.Rect.Top <= field.Rect.Bottom {
		t.Fatalf("unexpected field rect: %#v", field.Rect)
	}
	if field.ReadOnly || field.Required || field.NoExport {
		t.Fatalf("unexpected flags: %#v", field)
	}
}

func TestServiceFillFormUpdatesTextFieldValue(t *testing.T) {
	path := filepath.Join(t.TempDir(), "form.pdf")
	if err := os.WriteFile(path, buildTextFieldPDF("full_name", "Full Name", "Alice"), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
...
func TestServiceFillFormRejectsUnknownField(t *testing.T) {
	path := filepath.Join(t.TempDir(), "form.pdf")
	if err := os.WriteFile(path, buildTextFieldPDF("full_name", "Full Name", "Alice"), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
	...
	if err == nil {
		t.Fatal("expected unknown field fill to fail")
	}
	if !strings.Contains(err.Error(), "missing_field") {
		t.Fatalf("error = %v, want missing field name", err)
	}
}

func TestServiceFillFormUpdatesComboFieldSelection(t *testing.T) {
...
func TestServiceFillFormRejectsUnknownComboOption(t *testing.T) {
	path := filepath.Join(t.TempDir(), "combo.pdf")
	if err := os.WriteFile(path, buildComboFieldPDF("favorite_color", "Favorite Color", "Red", []string{"Red", "Green", "Blue"}), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
	...
	if err == nil {
		t.Fatal("expected unknown combo option fill to fail")
	}
	if !strings.Contains(err.Error(), "Purple") {
		t.Fatalf("error = %v, want option label in error", err)
	}
}

func TestServiceFillFormUpdatesCheckboxState(t *testing.T) {
...
func TestServiceFillFormRejectsInvalidCheckboxValue(t *testing.T) {
	path := filepath.Join(t.TempDir(), "checkbox.pdf")
	if err := os.WriteFile(path, buildCheckboxFieldPDF("subscribe", "Subscribe", false), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
...
func TestServiceFillFormUpdatesListFieldSelection(t *testing.T) {
	path := filepath.Join(t.TempDir(), "list.pdf")
	if err := os.WriteFile(path, buildListFieldPDF("priority", "Priority", "Medium", []string{"Low", "Medium", "High"}), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
	...
	if !reflect.DeepEqual(result.UpdatedFields, []string{"priority"}) {
		t.Fatalf("updated_fields = %#v, want [priority]", result.UpdatedFields)
	}
	outputPath := filepath.Join(t.TempDir(), "list-filled.pdf")
	if err := os.WriteFile(outputPath, result.Bytes, 0o644); err != nil {
		t.Fatalf("write filled pdf: %v", err)
	}
	...
	field := inspected.Fields[0]
	if field.Type != "list" {
		t.Fatalf("filled field type = %q, want list", field.Type)
	}
	if field.Value != "High" {
		t.Fatalf("filled list value = %q, want High", field.Value)
	}
	if len(field.Options) != 3 || !field.Options[2].Selected {
		t.Fatalf("filled list options = %#v, want High selected", field.Options)
	}
}

func TestServiceFillFormRejectsUnknownListOption(t *testing.T) {
	path := filepath.Join(t.TempDir(), "list.pdf")
	if err := os.WriteFile(path, buildListFieldPDF("priority", "Priority", "Medium", []string{"Low", "Medium", "High"}), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
	...
	if err == nil {
		t.Fatal("expected unknown list option fill to fail")
	}
...
func TestServiceFillFormUpdatesRadioSelection(t *testing.T) {
...
func TestServiceFillFormRejectsUnknownRadioExportValue(t *testing.T) {
	path := filepath.Join(t.TempDir(), "radio.pdf")
	if err := os.WriteFile(path, buildRadioFieldPDF("contact_method", "Contact Method", "Email", []string{"Email", "Phone"}), 0o644); err != nil {
		t.Fatalf("write pdf fixture: %v", err)
	}
