...
func TestStoreListByOwner(t *testing.T) {
	...
	if len(list) != 3 {
		t.Errorf("got %d reminders, want 3", len(list))
	}
...
func TestStoreListPending(t *testing.T) {
	...
	ctx := context.Background()
	store.Create(ctx, &Reminder{ID: "rem-1", OwnerID: "u1", Message: "A", FireAt: time.Now()})
	store.Create(ctx, &Reminder{ID: "rem-2", OwnerID: "u2", Message: "B", FireAt: time.Now()})
