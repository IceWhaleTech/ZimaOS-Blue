...
// SkillExecutor adapts deep search service to builtin skill executor interface.
...
// Execute runs deep search synchronously for skill invocation and returns a map result.
	...
	for {
		...
		case <-ticker.C:
			...
			case JobStatusFailed:
				if current.Error == "" {
					return nil, fmt.Errorf("deep search failed")
				}
				return nil, fmt.Errorf("deep search failed: %s", current.Error)
			case JobStatusCancelled:
				return nil, fmt.Errorf("deep search cancelled")
			}
		}
	}
}
