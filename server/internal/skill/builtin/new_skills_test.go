...
// --- Mock AutoreplyService ---

type mockAutoreplyService struct {
	rules map[string]AutoreplyRuleInfo
}

func newMockAutoreplyService() *mockAutoreplyService {
	return &mockAutoreplyService{rules: make(map[string]AutoreplyRuleInfo)}
}

func (m *mockAutoreplyService) Create(name, triggerType, triggerValue string, responses []string, priority int) (AutoreplyRuleInfo, error) {
	id := fmt.Sprintf("rule-%d", len(m.rules)+1)
	rule := AutoreplyRuleInfo{
		ID: id, Name: name, TriggerType: triggerType, TriggerValue: triggerValue,
		Responses: responses, Priority: priority, Enabled: true,
	}
	m.rules[id] = rule
	return rule, nil
}

func (m *mockAutoreplyService) List() []AutoreplyRuleInfo {
	var out []AutoreplyRuleInfo
...
func (m *mockAutoreplyService) Delete(id string) error {
...
func (m *mockAutoreplyService) Enable(id string) error {
	r, ok := m.rules[id]
	if !ok {
		return fmt.Errorf("rule not found: %s", id)
	}
...
func (m *mockAutoreplyService) Disable(id string) error {
	r, ok := m.rules[id]
	if !ok {
		return fmt.Errorf("rule not found: %s", id)
	}
...
func (m *mockAutoreplyService) Test(message, _ string) (string, string, error) {
	for _, r := range m.rules {
		if r.Enabled && r.TriggerType == "keyword" && r.TriggerValue == message {
			return r.Responses[0], r.ID, nil
		}
	}
	return "", "", nil
}
...
func TestAutoReply(t *testing.T) {
	ar := NewAutoReply()
	mock := newMockAutoreplyService()
	ar.SetAutoreplyService(mock)

	t.Run("manifest", func(t *testing.T) {
		if ar.Manifest().ID != "autoreply" {
			t.Errorf("expected ID 'autoreply', got '%s'", ar.Manifest().ID)
		}
	})

	t.Run("validate", func(t *testing.T) {
		if err := ar.Validate(map[string]any{}); err == nil {
			t.Error("expected error for missing action")
		}
		if err := ar.Validate(map[string]any{"action": "create"}); err == nil {
			t.Error("expected error for missing fields")
		}
		if err := ar.Validate(map[string]any{"action": "test"}); err == nil {
			t.Error("expected error for missing message")
		}
		if err := ar.Validate(map[string]any{"action": "delete"}); err == nil {
			t.Error("expected error for missing id")
		}
	})

	t.Run("create_list_test", func(t *testing.T) {
		result, err := ar.Execute(context.Background(), map[string]any{
			"action": "create", "name": "greeting",
			"trigger_type": "keyword", "trigger_value": "hello",
			"responses": []interface{}{"Hi there!", "Hey!"},
		})
		if err != nil || !result.Success {
			t.Fatalf("create failed: err=%v success=%v", err, result.Success)
		}

		result, err = ar.Execute(context.Background(), map[string]any{"action": "list"})
		if err != nil || !result.Success {
			t.Fatalf("list failed: err=%v", err)
		}
		if result.Data.(map[string]any)["count"] != 1 {
		...
		if result.Data.(map[string]any)["matched"] != true {
			t.Error("expected match")
		}

		// Test non-matching
		result, err = ar.Execute(context.Background(), map[string]any{
			"action": "test", "message": "goodbye",
		})
		if err != nil || !result.Success {
			t.Fatalf("test failed: err=%v", err)
		}
		if result.Data.(map[string]any)["matched"] != false {
			t.Error("expected no match")
		}
	})

	t.Run("disable_enable_delete", func(t *testing.T) {
		result, _ := ar.Execute(context.Background(), map[string]any{"action": "disable", "id": "rule-1"})
		if !result.Success {
			t.Error("disable failed")
		}
		result, _ = ar.Execute(context.Background(), map[string]any{"action": "enable", "id": "rule-1"})
		if !result.Success {
			t.Error("enable failed")
		}
		result, _ = ar.Execute(context.Background(), map[string]any{"action": "delete", "id": "rule-1"})
		if !result.Success {
	...
	t.Run("empty_responses", func(t *testing.T) {
		result, _ := ar.Execute(context.Background(), map[string]any{
			"action": "create", "name": "bad",
			"trigger_type": "keyword", "trigger_value": "x",
			"responses": []interface{}{},
		})
		if result.Success {
			t.Error("expected failure for empty responses")
		}
	})
