...
var (
	allLangKeywords         map[string]*langKeywords
	compiledIRLangKeywords  map[string]*compiledLangKeywords
	mediaIntentKeywordsOnce sync.Once
)

func ensureMediaIntentKeywords() {
	mediaIntentKeywordsOnce.Do(func() {
		allLangKeywords = buildAllLangKeywords()
		compiledIRLangKeywords = compileLangKeywords(allLangKeywords)
	})
}

// allLangKeywords maps language prefixes to their keyword sets.
// Covers all 27 supported locales grouped by language family.
func buildAllLangKeywords() map[string]*langKeywords {
	return map[string]*langKeywords{
		"en": {
