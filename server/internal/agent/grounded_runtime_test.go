...
func TestGroundedRuntimeWriteLSReadPasses(t *testing.T) {
	llmStub := &groundedScriptLLM{
		plannerResponses: []string{
			`{"status":"continue","reason":"Create the file.","next_tool":{"tool":"write_file","args":{"path":"demo.txt","content":"hello"}},"assertions":[]}`,
			`{"status":"continue","reason":"List the directory.","next_tool":{"tool":"ls","args":{"path":".","max_depth":1}},"assertions":[]}`,
			`{"status":"continue","reason":"Read the file.","next_tool":{"tool":"read_file","args":{"path":"demo.txt"}},"assertions":[]}`,
