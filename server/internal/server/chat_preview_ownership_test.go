...
func TestChatHandlerGetMessages_PreviewUserCanAccessLegacyConversation(t *testing.T) {
	...
	const convID = "legacy-conversation"
	if _, err := store.CreateConversationWithID(context.Background(), convID, "Legacy conversation"); err != nil {
...
func TestChatHandlerGetMessages_RegularUserStillCannotAccessLegacyConversation(t *testing.T) {
	store, err := memory.NewStore(":memory:")
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}
