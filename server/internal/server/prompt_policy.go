...
func policyFingerprint(profile string) string {
	...
	case DefaultPromptPolicyProfile:
		return "tool_guidance_v8|initial_agent_turn_v1|toolless_nudge_v12|post_tool_nudge_v5|context_rules_v1"
	default:
		return "tool_guidance_v8|initial_agent_turn_v1|toolless_nudge_v12|post_tool_nudge_v5|context_rules_v1"
	}
}
...
func (p PromptPolicy) ToolGuidanceConstraints() string {
	return "Tool guidance constraints: (1) Never print tool-call syntax as plain text (no {\"cmd\":...}, {\"command\":...}, ```tool, <exec>). (2) When using tools, emit structured tool_calls only, with valid JSON arguments. (3) One step at a time: prefer one high-confidence tool call per round, then wait for results. (4) Only use listed tools; do not invent capabilities. (5) For exec, arguments must contain a concrete non-empty command without placeholders. (6) If tools are unavailable or unnecessary, provide direct executable steps instead of fake calls. (7) Built-in tools first; MCP fallback only when needed. (8) External CLI skills are not native `blue <skill>` subcommands; when a skill manual documents a terminal binary, run it through `blue exec command='...'` instead of inventing commands like `blue summarize`. (9) Treat documentation-only examples and removed legacy helper names as docs, not live runtime tool targets. (10) Prefer `web_query` as the unified public-web tool; `web_search`, `web_fetch`, and `web_read` are compatibility aliases only when exposed. For auth-gated readable pages, retry with `Authorization`, `Cookie`, or `browser_target_id` reuse before full browser handoff; use browser or relay/local Chrome when live login, challenges, or interaction are truly required. (11) For reminder requests, call `reminder` directly with action/message/time and do not run `blue reminder --help`. (12) For large file writes, prefer transactional tools when available: use `write_begin`, then `write_chunk`, then `write_commit`. Otherwise never send one huge `write` payload: write the first chunk, then continue with smaller chunks using `append=true`. (13) For a desktop chat or messaging task, prefer `computer_use` high-level scenario actions like `message`, `select`, and `type` over screenshots or generic low-level host actions. Do not invent unsupported `computer_use` action names like `activate`, `mouse_click`, `move_to`, `accessibility_tree`, or `ocr`. Also do not use `open`, `open_location`, `list_apps`, browser `read`, or top-level `press`; if you need a shortcut or submit key, use `key` with `keys`/`submit_keys`. For `message`, always include `value`. Use `screenshot` only for evidence capture or last-resort debugging, not as the default control path."
}

func (p PromptPolicy) InitialAgentModeTurnNudge() string {
	return "Agent-mode loop policy: for open-ended or smart-resume continuation work, start with a bounded discovery pass using the currently exposed research-family tool or equivalent visible discovery route before drafting a broad long-range plan. Use the fresh discovery evidence to shape the initial canonical TODO checklist and the later execution order. " + p.ToolGuidanceConstraints()
}
...
func (p PromptPolicy) ToollessAutoContinueNudge(agentMode bool, reason string) string {
	if agentMode {
		...
		if reason == "missing_todo" {
			return "Agent mode checklist bootstrap required. FIRST output a canonical TODO checklist using markdown checkboxes (`- [ ] step`) for all remaining concrete tasks. For open-ended or continuation work, make the first unchecked item a bounded discovery pass with the currently exposed research-family tool or equivalent visible discovery route before broad long-range planning, and use that fresh evidence to shape the later checklist items. Then execute the first unchecked item with tools. Reprint the full checklist with updated checkbox states before each new action or summary, and continue until all items are checked. " + p.ToolGuidanceConstraints() + " Stop only if the user explicitly asks to stop."
		}
		if reason == "pending_todo" {
			return "A canonical TODO checklist already exists. In this turn, FIRST re-output the full checklist with updated checkbox states. If the task has not yet been grounded with fresh discovery, prefer a bounded discovery step with the currently exposed research-family tool or equivalent visible discovery route next before broadening the long-range checklist. Then execute the first unchecked item immediately by emitting at least one real tool call (prefer `exec`). If no tool is needed, provide a concrete completion summary with deliverables. Keep the checklist canonical by rewriting the full checklist on each progress turn so status stays accurate. " + p.ToolGuidanceConstraints() + " Stop only if the user explicitly asks to stop."
		}
		...
		return "You described what to do but did not call any tools. Now actually execute by calling available tools (especially exec for file creation/edit/run steps). Do not describe - act. For open-ended or smart-resume continuation work, prefer a bounded discovery pass first with the currently exposed research-family tool or equivalent visible discovery route to gather fresh context before expanding long-range planning. " + p.ToolGuidanceConstraints() + " Keep agent mode in a continuous improvement loop: after each completed action, find the next concrete improvement and execute it while continuing from the existing canonical TODO checklist. Update checklist status first, and reprint the full checklist with updated checkbox states before the next action or summary. Stop only if the user explicitly asks to stop."
	}
	if reason == "summary_intro" {
...
func (p PromptPolicy) PostToolAutoContinueNudge(agentMode bool) string {
	if agentMode {
		return "Continue the agent loop using the existing canonical TODO checklist. The tools above have been executed successfully. Review the results. If the latest evidence came from a bounded discovery pass, tighten the remaining long-range plan with that evidence before broadening execution. Then reprint the full checklist with updated checkbox states, identify the next concrete improvement opportunity for the next unchecked TODO, execute it, and repeat. " + p.ToolGuidanceConstraints() + " Stop only if the user explicitly asks to stop."
	}
