...
func (c *Controller) SyncExperimentGroupProjection(ctx context.Context, runID string) error {
	...
	if run.Kind != RunKindResearch {
		return nil
	}
	routeMode := strings.ToLower(strings.TrimSpace(metadataString(run.Metadata, "route_mode")))
	if routeMode != string(RunGroupKindExperiment) && routeMode != "hybrid" {
		return nil
	}

	groupID := experimentProjectionGroupID(runID)
	...
	if err == sql.ErrNoRows {
		group = &RunGroup{
			ID:          groupID,
			Kind:        RunGroupKindExperiment,
			Title:       projectionTitleForRun(run),
			Status:      projectionGroupStatusForRun(run),
			...
			SchedulerConfig: GroupSchedulerConfig{
				...
				RetryBackoff:   defaultGroupRetryBackoff,
			},
			ScoringConfig: GroupScoringConfig{
				Mode:          ScoringModeRule,
				RuleProfile:   "research_experiment_projection",
				PassThreshold: defaultGroupPassThreshold,
			},
			CreatedAt: now,
			UpdatedAt: now,
		}
		...
		item := RunGroupItem{
			ID:          experimentProjectionItemID(runID),
			GroupID:     group.ID,
			Index:       0,
			RunKind:     RunKindResearch,
			Profile:     "research_experiment_projection",
			Input:       map[string]interface{}{"goal": run.Goal},
			Expected:    map[string]interface{}{"status": string(RunStatusCompleted)},
			Status:      projectionItemStatusForRun(run),
			LatestRunID: run.ID,
			AttemptCount: func() int {
				if run.AttemptIndex > 0 {
		...
		if err := c.store.CreateGroupItems(ctx, []RunGroupItem{item}); err != nil {
			return err
		}
	} else {
		group.Status = projectionGroupStatusForRun(run)
		if group.Summary == nil {
			group.Summary = map[string]interface{}{}
		}
		group.Summary["projection"] = true
		group.Summary["route_mode"] = routeMode
		group.Subject = run.Goal
		if err := c.store.UpdateGroup(ctx, group); err != nil {
			return err
		}
		item, itemErr := c.store.GetGroupItem(ctx, experimentProjectionItemID(runID))
		if itemErr == nil {
			item.Status = projectionItemStatusForRun(run)
			item.LatestRunID = run.ID
			if run.AttemptIndex > item.AttemptCount {
				item.AttemptCount = run.AttemptIndex
			}
	...
	scorecard := buildProjectionScorecard(groupID, experimentProjectionItemID(runID), run)
	return c.store.AttachScorecard(ctx, scorecard)
}
...
func experimentProjectionGroupID(runID string) string {
	return "research-experiment:" + strings.TrimSpace(runID)
}

func experimentProjectionItemID(runID string) string {
	return experimentProjectionGroupID(runID) + ":item:0"
}

func projectionTitleForRun(run *Run) string {
	if run == nil {
		return "Research Experiment Projection"
	}
	goal := strings.TrimSpace(run.Goal)
	if goal == "" {
		return "Research Experiment Projection"
	}
	return "Research Experiment: " + goal
}

func projectionGroupStatusForRun(run *Run) RunGroupStatus {
	if run == nil {
		return RunGroupStatusFailed
	}
	switch run.Status {
...
func projectionItemStatusForRun(run *Run) RunGroupItemStatus {
	if run == nil {
	...
	case RunStatusCancelled:
		return RunGroupItemStatusCancelled
	case RunStatusFailed, RunStatusAborted:
		return RunGroupItemStatusError
	default:
		return RunGroupItemStatusRunning
	}
}

func buildProjectionScorecard(groupID, itemID string, run *Run) Scorecard {
	score := 0.0
