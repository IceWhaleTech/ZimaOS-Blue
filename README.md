...
> CLI rows above are the historical `help` / `status` microbenchmark on the same host. The fresh `gateway run` row is the real-world cold-idle memory checkpoint, measured on macOS arm64 via `vmmap Physical footprint` after startup settles. Feb-Apr 2026.
...
**Harness Evaluation Flow**
```
Quick Eval / Harness API → Eval Controller → Run Group Dispatcher
  → Agent Task or Eval Driver → Tools + Workspace + Artifacts
